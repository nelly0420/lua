...
function scene:create(event)
    ...
    local Data = jsonParse("json/prolog.json")
    ...
    local bgm -- Declare the background music variable

-- Play the background music (loops indefinitely)
local function playbgm(filename)
    if filename and filename ~= "" then
...
local function nextScript(event)
    ...
    if bgm then
        audio.play(bgm, { loops = -1 }) -- Loop the BGM indefinitely
    end
end
